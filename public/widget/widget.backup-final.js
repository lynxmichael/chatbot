(function () {
    ...
    */
    ...
    function updateCallStatusUI(status, message = "") {
        ...
        if (statusElement) {
            const messages = {
                connecting: "📞 Connexion avec un agent...",
                answered: "📞 Appel en cours avec un agent.",
                cancelled: "Appel terminé.",
                missed: "Aucun agent n'a pu répondre.",
                busy: "Tous les agents sont actuellement occupés.",
                failed: "Impossible de connecter l'appel."
            };

            statusElement.textContent =
                message || messages[status] || "Connexion...";
        }

        if (callButton) {
            callButton.style.display =
                status === "answered" && activeCallId
                    ? "none"
                    : "inline-block";
        }
